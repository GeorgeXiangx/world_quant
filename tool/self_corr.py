...
def calc_self_corr(
        ...
        os_alpha_ids: Optional[Dict[str, str]] = None,
    ...
    print(os_alpha_rets[os_alpha_ids[alpha_result['settings']['region']]].corrwith(alpha_rets).sort_values(
        ascending=False).round(4))
    os_alpha_rets[os_alpha_ids[alpha_result['settings']['region']]].corrwith(alpha_rets).sort_values(
        ascending=False).round(4).to_csv(str(cfg.data_path / 'os_alpha_corr.csv'))
...
class cfg:
    ...
    # data_path = Path('./wq_data')
    data_path = Path(__file__).parent.parent / 'wq_data'
